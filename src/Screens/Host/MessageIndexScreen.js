...
const MessageIndexScreen = (props) => {
  ...
  const [Loder, setLoader] = useState(false);
   ...
   useEffect(()=>{
    ...
    const onValueChange = database().ref('chat_history')
                            ...
                            style={{
                                ...
                                />
                                   ...
                                   />
                                    <Text 
                                    numberOfLines={1}
                                    style={{ color:item?.from != userDetails.id && item?.readStatus == "unread" ? '#000' : '#aaa', fontSize: 13, fontFamily:item?.from != userDetails.id && item?.readStatus == "unread" ? Theme.FontFamily.semiBold :Theme.FontFamily.normal,maxWidth:width/2.5,marginLeft:5,
                                     marginVertical: Platform.OS === 'ios' ? 3 : 2 }}>{item.msgType.toLowerCase()}...</Text>
                                   </View>
                                   :
                                        <Text 
                                    numberOfLines={1}
                                    style={{ color:item?.from != userDetails.id && item?.readStatus == "unread" ? '#000' : '#aaa', fontSize: 13, fontFamily:item?.from != userDetails.id &&item?.readStatus == "unread" ? Theme.FontFamily.semiBold :Theme.FontFamily.normal,
                                    maxWidth:width/2.1,
