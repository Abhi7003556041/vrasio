...
const SearchMapCard = (props) => {
  ...
  const changeLoc = useCallback((val) => {
    if(props?.latt!="" && props?.longg!=""){
      setRegion({...region,latitude:parseFloat(props?.latt),longitude:parseFloat(props?.longg), latitudeDelta: 10.922,
        longitudeDelta: 10.421,})
        
    setLoader(false)
 
    }
  }, [props?.latt,props?.longg])
  useEffect(() => { //add
  ...
  useEffect(() => {
    ...
    console.log('layatat',props?.latt,props?.longg)
    // sendLocation();
    // setLoader(false)
    // setTimeout(()=>{
      changeLoc()
      // if(props?.latt!="" && props?.longg!=""){
      //   setRegion({...region,latitude:parseFloat(props?.latt),longitude:parseFloat(props?.longg), latitudeDelta: 5.922,
      //     longitudeDelta: 5.421,})
  ...
  if (Loder) {
    return (
      <View style={{flex: 1, width: width - 30, height: 200, borderRadius: 10,  marginTop: 0,
  ...
  return (
    <View style={{ flex: 1, width: width - 30, height:width>700? 350: 250, borderRadius: 10, overflow: 'hidden', marginTop: 0 }}>
      {/* <BottomSheet> */}
      <MapView
        scrollEnabled={true}
        style={{
